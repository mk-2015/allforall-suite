...
{
    randalg_t* rhandle = (randalg_t*)malloc(sizeof(randalg_t));
    if (!rhandle) {
        last_error = OPEN_RAND_FAILMALLOC;
        return NULL;
    }

    rhandle->random_state = (uint64_t*)malloc(STATE_SIZE * sizeof(uint64_t));
    ...
    if (pthread_mutex_init(&rhandle->lock, NULL) != 0) {
        ...
        free(rhandle);
        last_error = OPEN_RAND_FAILMUTEX;
        return NULL;
    }
...
{
    if (!randalg) {
        last_error = GENRAND_FAILINVLAIDSTRUCT;
        return;
    }
    ...
    switch(randalg->algorithm) {
        ...
        default:
            ...
            last_error = GENRAND_FAILINVLAIDALGORITHM;
            return;
    }
...
{
    if (!randalg) {
        last_error = GETRAND_FAILINVLAIDSTRUCT;
        return 0;
    }
...
{
    if (!randalg) {
        last_error = SETOPT_FAILINVLAIDSTRUCT;
        return;
    }
    if (pthread_en) pthread_mutex_lock(&randalg->lock);
...
{
    if (!randalg) {
        last_error = RESET_FAILINVLAIDSTRUCT;
        return;
    }
    if (pthread_en) pthread_mutex_lock(&randalg->lock);
...
{
    if (!randalg) {
        last_error = CLOSERAND_FAILINVLAIDSTRUCT;
        return;
    }
