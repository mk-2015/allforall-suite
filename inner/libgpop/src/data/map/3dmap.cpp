...
int Map3D_Save(const char* filename, Map3D* Map) {
    if (!filename || !Map || !Map->Map) return -1;
    FILE* f = fopen(filename, "wb");
    if (!f) return -1;

    uint64_t header[3] = {(uint64_t)Map->x, (uint64_t)Map->y, (uint64_t)Map->z};
    fwrite(header, sizeof(uint64_t), 3, f);

    for (pos z = 0; z < Map->z; z++) {
        for (pos y = 0; y < Map->y; y++) {
            fwrite(Map->Map[z][y], sizeof(uint8_t), (size_t)Map->x, f);
        }
    }
...
int Map3D_Load(const char* filename, Map3D** Map) {
    if (!filename || !Map) return -1;
    FILE* f = fopen(filename, "rb");
    if (!f) return -1;

    uint64_t header[3];
    if (fread(header, sizeof(uint64_t), 3, f) != 3) {
        fclose(f);
        return -1;
    }

    *Map = Map3D_Init((pos)header[0], (pos)header[1], (pos)header[2]);
    if (!*Map) {
        fclose(f);
        return -1;
    }
    ...
    for (pos z = 0; z < (*Map)->z; z++) {
        for (pos y = 0; y < (*Map)->y; y++) {
            if (fread((*Map)->Map[z][y], sizeof(uint8_t), (size_t)(*Map)->x, f) != (size_t)(*Map)->x) {
                Map3D_Deinit(*Map);
                *Map = NULL;
                fclose(f);
                return -1;
