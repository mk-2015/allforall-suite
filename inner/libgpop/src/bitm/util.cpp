#include <bitm/util.hpp>
using namespace gpop::BitManipulation;
#include <stdint.h>

static const unsigned char BitReverseTable256[256] = {
    
};

COUNT bitcount(const unsigned char* bytes, size_t len) {
    if (!bytes) return 0;
    COUNT count = 0;
    for (size_t i = 0; i < len; i++) {
#ifdef __GNUC__
        count += __builtin_popcount(bytes[i]);
#else
        unsigned char b = bytes[i];
        while (b) {
            b &= (b - 1);
            count++;
        }
#endif
    }
    return count;
}

COUNT bytecount(const unsigned char* bytes, size_t len) {
    if (!bytes) return 0;
    COUNT count = 0;
    for (size_t i = 0; i < len; i++) {
        if (bytes[i] != 0) {
            count++;
        }
    }
    return count;
}

COUNT bitreverse(const unsigned char* in, unsigned char* out, size_t len) {
    if (!in || !out || len == 0) return 0;
    COUNT changed_bits = 0;

    for (size_t i = 0; i < len; i++) {
        unsigned char original = in[i];
        unsigned char reversed = BitReverseTable256[original];

        out[i] = reversed;

        unsigned char diff = original ^ reversed;
#ifdef __GNUC__
        changed_bits += __builtin_popcount(diff);
#else
        while (diff) {
            diff &= (diff - 1);
            changed_bits++;
        }
#endif
    }
    return changed_bits;
}

COUNT bytereverse(const unsigned char* in, unsigned char* out, size_t len) {
    if (!in || !out || len == 0) return 0;
    COUNT changed_bits = 0;

    size_t left = 0;
    size_t right = len - 1;

    while (left <= right) {
        unsigned char left_val = in[left];
        unsigned char right_val = in[right];

        unsigned char diff = left_val ^ right_val;
        if (left != right) {
#ifdef __GNUC__
            changed_bits += __builtin_popcount(diff);
#else
            while (diff) {
                diff &= (diff - 1);
                changed_bits++;
            }
#endif
        }

        out[right] = left_val;
        out[left] = right_val;

        if (right == 0 || left >= right) break;
        left++;
        right--;
    }

    return changed_bits;
}
