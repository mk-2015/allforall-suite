...
void wipeFileContents(const fs::path &filePath) {
  std::ofstream outFile(filePath, std::ios::in | std::ios::out |
  ...
  while (remainingBytes > 0) {
    std::streamsize toWrite = std::min(
        remainingBytes, static_cast<std::streamsize>(zeroBuffer.size()));
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::cout << "Successfully deleted target directory and " << deletedCount
              << " items.\n";
  } catch (const fs::filesystem_error &e) {
